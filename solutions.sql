--- Query1: Get the `id` values of the first 5 clients from `district_id` with a value equals to 1.
...
--- Query6: What is the loan `amount` of the lowest 5 `account_id`s in the `loan` table? Show the `account_id` and the corresponding `amount`
SELECT account_id, amount FROM loan ORDER BY account_id ASC LIMIT 5;

--- Query7: What are the `account_id`s with the lowest loan `amount` that have a loan `duration` of 60 in the `loan` table?
SELECT account_id FROM loan WHERE duration = 60 ORDER BY amount ASC LIMIT 5;

--- Query8: What are the unique values of `k_symbol` in the `order` table? Note: There shouldn't be a table name `order`, since `order` is reserved from the `ORDER BY` clause. You have to use backticks to escape the `order` table name. 
SELECT DISTINCT k_symbol FROM `order` WHERE k_symbol != " " ORDER BY k_symbol ASC;

--- Query9: In the `order` table, what are the `order_id`s of the client with the `account_id` 34?
SELECT order_id FROM `order` WHERE account_id = 34 ORDER BY order_id ASC;

--- Query10: In the `order` table, which `account_id`s were responsible for orders between `order_id` 29540 and `order_id` 29560 (inclusive)?
...
--- Query15: Using the `loan` table, print the top 10 `account_id`s based on the sum of all of their loan amounts.
...
SELECT date,  duration, Count(*) AS loan_count  FROM loan WHERE date LIKE '9712%' GROUP BY date, duration ORDER BY date ASC, duration ASC;
