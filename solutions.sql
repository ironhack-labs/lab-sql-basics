## Exercice 1

SELECT client_id FROM client WHERE district_id == 1 LIMIT 5;

## Exercice 2

SELECT client_id FROM client WHERE district_id == 72 ORDER BY client_id DESC LIMIT 1;

## Exercice 3

SELECT amount FROM loan ORDER BY amount ASC LIMIT 3;

## Exercice 4

SELECT DISTINCT status FROM loan ORDER BY status ASC;

## Exercice 5

SELECT loan_id FROM loan ORDER BY payments DESC LIMIT 1;

## Exercice 6

SELECT account_id, amount FROM loan ORDER BY account_id ASC LIMIT 5;

## Exercice 7

SELECT account_id FROM loan WHERE duration == 60 ORDER BY amount ASC LIMIT 5;

## Exercice 8

SELECT DISTINCT k_symbol FROM `order` WHERE TRIM (k_symbol) <> '' AND k_symbol IS NOT NULL ORDER BY k_symbol ASC;

## Exercice 9

SELECT order_id FROM `order` WHERE account_id == 34;

## Exercice 10

SELECT DISTINCT account_id FROM `order` WHERE order_id BETWEEN 29540 AND 29560 ORDER BY account_id ASC;

## Exercice 11

SELECT amount FROM `order` WHERE account_to == 30067122;

## Exercice 12

SELECT trans_id, date, type, amount FROM trans WHERE account_id == 793 ORDER BY date DESC LIMIT 10;

## Exercice 13

SELECT count(client_id) FROM client WHERE district_id < 10 GROUP BY district_id ORDER BY district_id;

## Exercice 14

SELECT type, COUNT(*) AS total_cards FROM card GROUP BY type ORDER BY total_cards DESC;

## Exercice 15

SELECT account_id, SUM(amount) AS total_loan FROM loan GROUP BY account_id ORDER BY  total_loan DESC LIMIT 10;

## Exercice 16

SELECT date, COUNT(*) AS loan_count FROM loan WHERE date < 930907 GROUP BY date ORDER BY date DESC;

## Exercice 17

SELECT date, duration, COUNT(*) AS loan_count FROM loan WHERE date BETWEEN 971201 AND 971231 GROUP BY date, duration ORDER BY date, duration ASC;

## Exercice 18

SELECT account_id, type, SUM(amount) AS total_amount FROM trans WHERE account_id == 396 GROUP BY account_id, type ORDER BY type ASC;