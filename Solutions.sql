...
SELECT *
FROM district d
JOIN client c ON d.A1 = c.district_id
WHERE d.A1 = 1
LIMIT 5;
...
ROUND(SUM(amount)) AS transaction_type
FROM trans
WHERE account_id = 396
GROUP BY 1, 2;
...
SELECT account_id, type, LISTAGG(ROUND(SUM(amount)), ' ') AS transaction_type
FROM trans
WHERE account_id = 396
GROUP BY 1, 2;

-- Continuing with the previous example, rank the top 10 account_ids based on their difference.
SELECT account_id, ROUND(SUM(amount))
FROM trans
WHERE account_id = 396
-- AND 
GROUP BY 1, 2;
