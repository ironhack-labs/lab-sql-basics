...
SELECT DISTINCT `date`, COUNT(*) FROM loan WHERE `date` < 930907 GROUP BY `date` ORDER BY COUNT(*) DESC;
SELECT DISTINCT `date`, duration, COUNT(*) FROM loan WHERE `date` BETWEEN "971201" AND "971231" GROUP BY `date`, duration ORDER BY `date` ASC, duration ASC; 
