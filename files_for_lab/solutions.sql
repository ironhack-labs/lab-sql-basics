...
SELECT CAST(amount AS INT) AS amount FROM `order` WHERE account_to == 30067122;
...
SELECT account_id, type, CAST(SUM(amount) AS DECIMAL(15,10)) AS total_amount FROM trans WHERE account_id == 396 GROUP BY type ORDER BY type;