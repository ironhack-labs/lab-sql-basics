...
-- Remark: The above was calculated assuming that we only want to know the top 5, complying with the desired output. Below is another solution that in my opinion matches the formulation of the task more closely, but only returns one item:
SELECT account_id
FROM loan
WHERE duration = 60
  AND amount = (
...
SELECT date, duration, count(*) FROM loan WHERE date >= 971200 AND date <= 971231 GROUP BY date, duration ORDER BY date, duration ASC;
