-- Query 1: Get the `Get the `id` values of the first 5 clients from `district_id` with a value equals to 1.` values of the first 5 clients from `district_id` with a value equals to 1.
...
SELECT order_id  FROM 'order' WHERE account_id = 34 ORDER BY account_id;
...
SELECT account_id, sum (amount) as suma FROM loan GROUP BY account_id ORDER By suma DESC LIMIT 10;
...
SELECT date, count(*) as total_by_day FROM loan WHERE date < 930907 GROUP BY date ORDER By date DESC;
...
SELECT account_id, type, sum (amount) as total_amount FROM trans WHERE account_id = 396 GROUP BY type ORDER BY account_id ASC;
