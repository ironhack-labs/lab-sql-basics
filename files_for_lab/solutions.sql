...
Get the `id` values of the first 5 
clients from `district_id` with a value equals to 1.
...
What is the loan `amount` of the lowest 5 `account_id`s in the `loan` table? 
...
In the `order` table, which `account_id`s were responsible for orders between 
...
In the `trans` table, show the `trans_id`, `date`, `type` and `amount` of the 10 first transactions from 
...
issued for each day, before (excl) 930907, ordered by date in descending order.
...
BETWEEN CAN BE CHANGED TO: WHERE date >= 971201 AND date <= 971231
