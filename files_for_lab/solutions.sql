...
SELECT payments FROM loan ORDER BY payments DESC LIMIT 1;
...
SELECT type, COUNT(card_id) AS number_of_cards FROM card GROUP BY type ORDER BY number_of_cards DESC;
...
SELECT date, COUNT(loan_id) AS number_of_loans FROM loan GROUP BY date ORDER BY date DESC;

--In the `loan` table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in ascending order. You can ignore days without any loans in your output.
SELECT date, duration, COUNT(loan_id) AS number_of_loans FROM loan WHERE date BETWEEN 971201 AND 971231 GROUP BY date, duration ORDER BY date ASC, duration ASC;
