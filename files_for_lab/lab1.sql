...
SELECT DISTINCT account_id, amount FROM loan order by account_id asc LIMIT 5;

--Q7 What are the account_ids with the lowest loan amount that have a loan duration of 60 in the loan table?

SELECT DISTINCT account_id FROM loan WHERE duration= 60 order by amount ASC LIMIT 5;
...
SELECT account_id, sum(amount) AS total_loans FROM loan GROUP by account_id order by total_loans DESC LIMIT 10;
...
SELECT date, duration, count(*) as loan_count FROM loan WHERE date BETWEEN 971201 and 971231 GROUP by date, duration order by date, duration;
