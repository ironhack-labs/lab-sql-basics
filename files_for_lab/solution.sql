...
Note: There shouldnt be a table name order, since order is reserved from the ORDER BY clause. You have to use backticks to escape the order table name.
...
SELECT account_id, SUM(amount) AS summ from loan GROUP BY account_id ORDER BY summ DESC LIMIT 10;

--16 In the loan table, retrieve the number of loans issued for each day, beforse (excl) 930907, ordered by date in descending order.
...
SELECT date, duration, COUNT(*) AS COUNT_LOANS from loan WHERE date BETWEEN 971201 and 971231 GROUP BY date, duration ORDER BY date ASC, duration ASC;
